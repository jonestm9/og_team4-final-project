...
class ServiceHandler (spb_grpc.DummyServiceServicer):
  ...
  def method(self, request, context):
      ...
      try:
          ...
          match request.type:
              case spb.MsgType.OrderType:
                print("Order message received with contents: \n{}".format(request))
                response.code = spb.ResponseCode.OK
                response.contents = spb.ContentsStatus.ORDER_PLACED
              case _:
                print("msgtype is {}, expected Order(2). sending response of BAD_REQUEST".format(request.type))
...
def parseCmdLineArgs ():
    ...
    parser.add_argument ("-p", "--port", type=int, default=5578, help="Port where the server part of the peer listens and client side connects to (default: 5577)")
