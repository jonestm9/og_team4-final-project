...
class ServiceHandler (spb_grpc.DummyServiceServicer):
  ...
  def method(self, request, context):
      ...
      try:
          ...
          print("Received request - timestamp: {}, type: {}".format(request.ts, request.type))

          print("creating response to send to client")
          resp = spb.Master()
          resp.type = spb.MsgType.ResponseType
          response = spb.Response()

          match request.type:
              case spb.MsgType.HealthType:
                print("Health message received with contents: \n{}".format(request))
                response.code = spb.ResponseCode.OK
                # check health is valid (whatever checks we want on hunidity, sensors etc)
                response.contents = spb.ContentsStatus.YOU_ARE_HEALTHY
              case _:
                print("msgtype is {}, expected Health(3). sending response of BAD_REQUEST".format(request.type))
                response.code = spb.ResponseCode.BAD_REQUEST
                response.contents = spb.ContentsStatus.BAD_REQ
          
          resp.responseBody.CopyFrom(response)
          print("sending response: \n{}".format(resp))
          return resp  # return the response to be sent to the client
      except:
...
def main ():
  ...
  print("Health Server for Protocol Buffers with gRPC serialization/deserialization")
